...
- Package version 0.2.14 uses protocol 0.1.10 and Android 0.4.7 (versionCode 11). Android exact-matches its generated supported version, distinguishes non-Postbox HTTP failures from protocol mismatches, starts saved-server workflows offline, refreshes stale Question revisions, and reconciles resolved-push badges offline and idempotently even without a fresh process cache or active child notification. This release also restores stamped Question Chat transport events, makes Answer auto-wake recovery queue-aware, preserves valid config fields, and derives Android contract inputs from protocol/server sources of truth.
