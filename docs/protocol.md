...
- Package version 0.2.12 uses protocol 0.1.10 and Android 0.4.5 (versionCode 9). Android exact-matches its generated supported version, distinguishes non-Postbox HTTP failures from protocol mismatches, starts saved-server workflows offline, refreshes stale Question revisions, and reconciles resolved-push badges offline even without a fresh process cache. This release also restores stamped Question Chat transport events, makes Answer auto-wake recovery queue-aware, preserves valid config fields, and derives Android contract inputs from protocol/server sources of truth.
