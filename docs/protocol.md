...
- Package version 0.2.10 uses protocol 0.1.9 and Android 0.4.3 (versionCode 7). Android exact-matches its generated supported version, versions every consumed envelope, and hard-blocks incompatible connection-scoped workflows before decoding or side effects. This release keeps current-owner status counts synchronized, makes the development API port 45795 and production API port 32187 strict by default, and adds a notification-backed Android launcher badge for the full pending queue.
