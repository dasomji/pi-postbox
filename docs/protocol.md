...
- Package version 0.2.11 uses protocol 0.1.10 and Android 0.4.4 (versionCode 8). Android exact-matches its generated supported version, distinguishes non-Postbox HTTP failures from protocol mismatches, starts saved-server workflows offline, and refreshes stale Question revisions. This release also restores stamped Question Chat transport events, makes Answer auto-wake recovery queue-aware, preserves valid config fields, reconciles resolved-push badges offline, and derives Android contract inputs from protocol/server sources of truth.
