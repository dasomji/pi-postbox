...
- Package version 0.5.2 uses protocol 0.1.11 and Android 0.5.2 (versionCode 14). Ordered Question galleries use extension-local paths, connection-bound binary staging, transactional revision references and immutable media reads. Full snapshots default omitted `images` to `[]`; create/batch/revise carry staged references, and compact discovery/status/Answer reads stay unchanged. See [Question image galleries](question-images.md). Existing Pi sessions require a full restart for the new shared protocol and tool schema.
