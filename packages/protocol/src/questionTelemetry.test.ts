...
describe("content-free Question telemetry contract", () => {
  it("records only lengths, aggregate sizes, batch/count dimensions, and Answer response sizes", () => {
    ...
    const event = schema.parse({ operation: "question.batch.create", questionLength: 42, ambiguityLength: 39,
      optionValueLength: 15, optionLabelLength: 16,
      optionDescriptionLength: 17, optionImpactLength: 18, noteLength: 4,
      optionsSerializedBytes: 320,
    ...
    expect(event).toMatchObject({ operation: "question.batch.create", questionLength: 42, ambiguityLength: 39,
      optionLabelLength: 16, optionImpactLength: 18,
